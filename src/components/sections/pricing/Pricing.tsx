...
import { Check, Star, Zap, Calculator, TrendingUp, Shield } from 'lucide-react';
...
const DEFAULT_PRICING = {
  ...
  mainTitleHighlight: 'Accounting Pricing',
  mainDescription:
    'Choose the perfect plan for your business accounting needs. No hidden fees, no setup costs. Start your free trial today.',
  billingMonthly: 'Monthly',
  billingAnnual: 'Annual',
  billingAnnualBadge: 'Save 25%',
  plan1Name: 'Starter',
  plan1Description: 'Perfect for freelancers and micro businesses',
  plan1Price: '$15',
  plan1Period: '/month',
  plan1CTA: 'Start Free Trial',
  plan1CTAHref: '/',
  plan2Name: 'Professional',
  plan2Description: 'Best for growing small businesses',
  plan2Price: '$39',
  plan2Period: '/month',
  ...
  plan3Description: 'For established businesses with complex needs',
  plan3Price: '$89',
  plan3Period: '/month',
  plan3Badge: 'Advanced',
  ...
  bottomDescription:
    'We offer tailored accounting packages for businesses with unique requirements. Get dedicated CPA support and custom integrations.',
...
export default function Pricing(props: PricingProps) {
  ...
  const getAnnualPrice = (monthlyPrice: string) => {
    if (monthlyPrice === 'Custom' || !monthlyPrice.includes('$')) return monthlyPrice;
    const price = parseInt(monthlyPrice.replace('$', ''));
    const annualPrice = Math.floor(price * 12 * 0.75); // 25% discount
    return `$${annualPrice}`;
  };

  const getAnnualPeriod = () => '/year';

  const plans = [
    {
      name: config.plan1Name,
      description: config.plan1Description,
      price: billingCycle === 'monthly' ? config.plan1Price : getAnnualPrice(config.plan1Price),
      period: billingCycle === 'monthly' ? config.plan1Period : getAnnualPeriod(),
      badge: null,
      icon: Calculator,
      features: [
    ...
    },
    {
      ...
      price: billingCycle === 'monthly' ? config.plan2Price : getAnnualPrice(config.plan2Price),
      period: billingCycle === 'monthly' ? config.plan2Period : getAnnualPeriod(),
    ...
    },
    {
      ...
      price: billingCycle === 'monthly' ? config.plan3Price : getAnnualPrice(config.plan3Price),
      period: billingCycle === 'monthly' ? config.plan3Period : getAnnualPeriod(),
